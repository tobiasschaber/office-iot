...
resource "aws_iot_topic_rule" "motion_detection_forwarder_rule" {
  ...
  enabled = false
  sql = "SELECT *, timestamp() AS creationTimestamp, ((timestamp()/1000)+(48*60*60)) as ttl, clientToken as sensorId, state.reported.motionDetected as motionDetected FROM '$aws/things/+/shadow/update' "
